...
```bash
git clone ssh://git@gitlab.loudmumble.com:2424/loudmumble/crack-ng.git
cd crack-ng
cargo build --release
```
...
- **`engine.rs`** -- GPU hardware detection, optimized parameter tuning, Hashcat/JtR process orchestration with async streaming. Uses tab separator for hashcat output to handle colon-containing hash formats (NetNTLMv2, LM:NT pairs).
...
```
ssh://git@gitlab.loudmumble.com:2424/loudmumble/crack-ng.git
```
