...
/// Falls back to potfile cross-referencing when `--show` returns `?` as the hash
/// (which happens for raw hash formats without usernames).
fn harvest_jtr_results(
    ...
    // Build a set of original hashes from the hash file for potfile cross-referencing.
    // Lowercased for case-insensitive matching against potfile entries.
    ...
    // Check if --show output contains `?` placeholders (raw hash formats).
    // When JtR shows `?:plaintext`, it means the hash file had no username field,
    // so we need to cross-reference with the potfile to recover the actual hashes.
    let has_placeholder = output.lines().any(|l| {
        let l = l.trim();
        !l.is_empty() && !l.contains("password hash") && l.starts_with("?:")
    });

    if has_placeholder {
        // Potfile cross-reference: read JtR potfiles and match against our hash file
        harvest_from_potfiles(s, job_idx, &original_hashes, algo_name);
    } else {
        // Standard parsing: hash:plaintext from --show output
        for line in output.lines() {
            let line = line.trim();
            if line.is_empty() || line.contains("password hash") {
                continue;
            }
...
/// Cross-reference JtR potfiles against a set of original hashes to recover
/// hash:plaintext mappings. JtR potfile entries may have format prefixes like
/// `$dynamic_0$hash:plaintext` that need to be stripped to match raw hashes.
fn harvest_from_potfiles(
    s: &mut CrackState,
    job_idx: usize,
    original_hashes: &std::collections::HashSet<String>,
    algo_name: &str,
) {
    let mut potfile_paths = Vec::new();

    // JtR stores pot in its run directory ($JOHN/john.pot)
    if let Ok(john_path) = std::process::Command::new("which")
        .arg("john")
        .output()
    {
        if john_path.status.success() {
            let john_bin = String::from_utf8_lossy(&john_path.stdout).trim().to_string();
            let john_dir = std::path::Path::new(&john_bin)
                .parent()
                .unwrap_or(std::path::Path::new("."));
            let pot = john_dir.join("john.pot");
            if pot.exists() {
                potfile_paths.push(pot);
            }
            // If it's a wrapper script, also check the actual binary's directory
            if let Ok(content) = std::fs::read_to_string(&john_bin) {
                for line in content.lines() {
                    if line.contains("exec ") || line.contains("john") {
                        // Extract path from "exec /path/to/john" pattern
                        for word in line.split_whitespace() {
                            if word.contains("/john") && !word.starts_with('#') {
                                let real_dir = std::path::Path::new(word)
                                    .parent()
                                    .unwrap_or(std::path::Path::new("."));
                                let pot = real_dir.join("john.pot");
                                if pot.exists() {
                                    potfile_paths.push(pot);
                                }
                            }
    ...
    if let Some(home) = home::home_dir() {
        let pot = home.join(".john").join("john.pot");
        if pot.exists() {
            potfile_paths.push(pot);
        }
    }
    ...
    for pot_path in &potfile_paths {
        let content = match std::fs::read_to_string(pot_path) {
            Ok(c) => c,
            Err(_) => continue,
        };
        for line in content.lines() {
            ...
            // Potfile format: [prefix$]hash:plaintext
            // Split on LAST colon to get plaintext
            if let Some(sep_pos) = line.rfind(':') {
                let hash_part = &line[..sep_pos];
                let plain_part = &line[sep_pos + 1..];
                if hash_part.is_empty() || plain_part.is_empty() {
                ...
                // Strip JtR format prefix (e.g. "$dynamic_0$" for raw-md5)
                // These prefixes follow the pattern: $format_name$actual_hash
                let raw_hash = strip_jtr_prefix(hash_part);
                if original_hashes.contains(&raw_hash.to_lowercase()) {
                    let record = ExportRecord {
...
/// Strip JtR format prefix from a potfile hash entry.
/// JtR prefixes hashes in the potfile with `$format$` identifiers:
///   `$dynamic_0$5f4dcc3b...` -> `5f4dcc3b...`
///   `$SHA256$abc123...` -> `abc123...`
/// For structured hashes ($6$, $2a$, $krb5tgs$, etc.) the prefix IS part of
/// the hash, so we only strip prefixes that JtR adds for "raw" hash formats
/// where the original input was a bare hex string.
fn strip_jtr_prefix(hash: &str) -> &str {
    if !hash.starts_with('$') {
        return hash;
    }
    // Known JtR potfile prefixes for raw/simple hash formats.
    // These are added by JtR and not part of the original hash.
    const RAW_PREFIXES: &[&str] = &[
        "$dynamic_",  // $dynamic_0$ (raw-md5), $dynamic_26$ (raw-sha1), etc.
        "$SHA256$",
        "$SHA512$",
        "$SHA384$",
        "$SHA$",      // raw-sha1 alternate
        "$MD5$",
        "$MD4$",
        "$NT$",
        "$LM$",
    ...
    for prefix in RAW_PREFIXES {
        if let Some(stripped) = hash.strip_prefix(prefix) {
            // For $dynamic_N$ the closing $ comes after the number
            if *prefix == "$dynamic_" {
                if let Some(end) = hash[1..].find('$') {
                    return &hash[end + 2..];
                }
            } else {
                return stripped;
...
pub async fn run_orchestrator(args: &Args, state: Arc<Mutex<CrackState>>) {
    ...
    for i in 0..num_jobs {
        ...
        // Resolve user wordlist: decompress if needed (engines can't read .gz/.bz2/.xz)
        let resolved_wordlist: Option<std::path::PathBuf> = args.wordlist.as_ref().map(|w| {
            if crate::wordlist::is_compressed(w) {
                crate::wordlist::decompress_path(w).unwrap_or_else(|| w.clone())
            } else {
                w.clone()
            }
        });
        ...
        let (cmd, is_hashcat) = if let (true, Some(mode)) = (use_gpu, &job.hashcat_mode) {
            ...
            if let Some(a) = &args.attack_mode {
                c.arg("-a").arg(a);
            } else if resolved_wordlist.is_some() {
                c.arg("-a").arg("0");
            } else {
                c.arg("-a").arg("3");
                c.arg("?a?a?a?a?a?a");
            ...
            if let Some(w) = &resolved_wordlist {
                c.arg(w);
            }
        ...
        if matches!(stage, CascadeStage::PotfileCheck) {
            ...
            for i in 0..num_jobs {
                ...
                if let Some((text, sep)) = show_output {
                    // Check for JtR `?` placeholder (raw hash formats without usernames)
                    let has_jtr_placeholder = sep == ':' && text.lines().any(|l| {
                        let l = l.trim();
                        !l.is_empty() && !l.contains("password hash") && l.starts_with("?:")
                    });

                    if has_jtr_placeholder {
                        // Use potfile cross-reference for raw hash formats
                        let original_hashes: std::collections::HashSet<String> =
                            std::fs::read_to_string(&hash_file)
                                .unwrap_or_default()
                                .lines()
                        ...
                        harvest_from_potfiles(&mut s, i, &original_hashes, &algo_name);
                        let stage_recovered = s.jobs[i].cracked - before;
                        if stage_recovered > 0 {
                            s.push_log(format!(
                                "[+] Potfile: recovered {} hashes for {} via potfile cross-reference",
                                stage_recovered, algo_name
                            ));
                        }
                    } else {
                        let mut s = state.lock().await;
                        ...
                        for line in text.lines() {
                            ...
                            } else if let Some(pos) = line.rfind(sep) {
                                (&line[..pos], &line[pos + 1..])
                            } else {
                                continue
                            };
